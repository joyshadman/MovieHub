...
const Player = ({ movie, user, onClose }) => {
  ...
  // Reset loading/error state AND re-arm the click shield whenever the
  // source, season, or episode changes (new iframe = fresh redirect risk)
  useEffect(() => {
    setLoading(true);
    setHasError(false);
    ...
    const timer = setTimeout(() => {
      setLoading((stillLoading) => {
        if (stillLoading) setHasError(true);
        return stillLoading;
      });
    }, 12000);
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 backdrop-blur-xl p-0 md:p-6">
      ...
      >
              ...
              <div className="grid grid-cols-2 gap-2">
                {providers.map((p) => (
                  <button
                    ...
                    className={`py-3 rounded-xl text-[9px] font-black uppercase transition-all border ${server === p.id ? 'bg-red-600 border-red-400 text-white' : 'bg-white/5 border-white/5 text-white/30 hover:text-white'
                      }`}
                  >
        ...
        <div className="relative flex-1 bg-black">
          ...
          {/* CLICK SHIELD - absorbs the first click (most common ad-redirect
              trigger) without blocking the player's own controls underneath.
              Tap once to dismiss; re-arms automatically on source/ep change. */}
          {shieldActive && (
            <div
              className="absolute inset-0 z-40 cursor-pointer bg-transparent"
              onClick={() => setShieldActive(false)}
              title="Tap to enable player controls"
            />
          )}
