...
const SearchPage = ({ onMovieClick, externalQuery = "" }) => {
  ...
  // Sync internal query with Navbar query
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      if (query.trim().length > 2) {
        ...
        try {
          ...
          // Standardize data structure if necessary
  ...
  return (
    <div className="min-h-screen bg-[#050505] pt-32 px-6 md:px-12 pb-20">
      <div className="max-w-4xl mx-auto mb-16">
        ...
        <div className="flex items-center gap-6 mb-12">
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            onClick={() => navigate(-1)}
            className="p-4 bg-white/[0.03] border border-white/10 rounded-2xl text-white/40 hover:text-white hover:bg-red-600 transition-all shadow-2xl"
          >
            <ArrowLeft size={20} />
          </motion.button>
          <div>
            <h2 className="text-white text-xs font-black uppercase tracking-[0.4em] opacity-30">Global Search</h2>
        ...
        >
          <div className="absolute -inset-1 bg-gradient-to-r from-red-600 to-violet-600 rounded-3xl blur opacity-10 group-focus-within:opacity-30 transition duration-500"></div>
          
          <div className="relative flex items-center bg-white/[0.02] border border-white/10 rounded-[2rem] px-8 py-6 backdrop-blur-3xl shadow-2xl transition-all group-focus-within:border-white/20">
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-3 px-8 py-3.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all border shrink-0 ${
                ...
                : 'bg-white/[0.03] border-white/10 text-white/30 hover:border-white/30 hover:text-white hover:bg-white/[0.08]'
              }`}
            >
              {btn.icon} {btn.label}
            </button>
      ...
      {/* RESULTS GRID WITH ANIMATED LAYOUT */}
        ...
        <AnimatePresence mode='wait'>
          {isLoading ? (
            ...
            <motion.div 
              ...
              className="text-center py-48 bg-white/[0.01] border border-dashed border-white/5 rounded-[4rem]"
            >
              ...
              <p className="text-white/10 text-[10px] mt-4 uppercase font-bold tracking-widest">Try adjusting your filters or search terms</p>
            </motion.div>
          ) : (
            <motion.div 
              key="idle" 
            ...
            >
              <div className="flex justify-center gap-4 mb-8 opacity-10">
                <Film size={40} className="text-white" />
                <Tv size={40} className="text-white" />
                <SearchIcon size={40} className="text-white" />
              </div>
              <p className="text-white/10 text-[11px] font-black uppercase tracking-[1em]">Scanning for signal...</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
